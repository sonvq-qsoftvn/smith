	...
	$.fn.youTubeEmbed = function(settings){
		...
		$.get(youtubeAPI,{'q':settings.videoID},function(response){
			...
			elements.control = $('<div>',{className:'controlDiv play'})
                        ...
                        // Creating the sound Div. It will act as a mute/unmute button

			elements.sound = $('<div>',{className:'soundDiv mute'})
							   .appendTo(elements.container);

            //create logo div
            elements.logo = $('<div>',{className:'logo'})
							   .appendTo(elements.container);
			...
			window['eventListener_'+settings.safeID] = function(status){
				...
				if(status==-1)	// video is loaded
				{
                                    ...
                                    elements.sound.click(function(){
                                                if(elements.sound.hasClass('muted')){
                                                    elements.player.unMute();
                                                    elements.sound.removeClass('muted').addClass('mute');
                                                }
                                                else{
                                                    elements.player.mute();
                                                    elements.sound.removeClass('mute').addClass('muted');
                                                }
                                                });
